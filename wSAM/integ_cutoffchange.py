...
thresh=0.1
wf=open(f"./Au144/{fappend}_res_cutoffchange.csv",'w')
...
for i1 in range(1,nmask):
    ...
    if min_distance<15:
        ...
        while cond:
            ...
            count+=1
            print(i1,count,intchange,sumcor_prev,sumcor_now,avg_now,avg_bkg,n_now,n_bkg,dilated_mask.sum())
            masknow=binary_dilation(masknow, structure=dilate_struct2)
            if abs(intchange)<thresh:
                cond=False
                #quit()
        tw=f"{i1},{centroid_col},{centroid_row},{closest_index+1},{min_distance},{n_now},{sumcor_now},{sum_now},{avg_now},{avg_bkg},{count}\n"
        wf.write(tw)
    """
